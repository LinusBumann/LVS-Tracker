defmodule LvsToolWeb.SemesterentryLive.ReductionsComponent do
  ...
  def render(assigns) do
    ...
    <div class="space-y-6">
      ...
      <div :if={Enum.count(@reductions) == 0} class="text-center py-12">
      ...
      <div :if={Enum.count(@reductions) > 0} class="bg-white shadow rounded-lg">
        <div class="px-4 py-5 sm:p-6">
          <div class="flow-root">
            ...
            >
              <li :for={{dom_id, reduction} <- @reductions} id={dom_id} class="py-4">
                <div class="flex items-center space-x-4">
                  <div class="flex-1 min-w-0">
                    <p class="text-sm font-medium text-gray-900 truncate">
                      {reduction.reduction_type.name}
                    </p>
                    
                    <p class="text-sm font-medium text-gray-500 truncate">
                      Thesistyp: {reduction.reduction_type.name}
                    </p>
                    
                    <p class="text-sm text-gray-500">
                      <span class="font-semibold">Studiengruppen:</span> {Enum.map(
                        reduction.studygroups,
                        fn type -> type.name end
                      )}
                    </p>
                    
                    <p class="text-sm text-gray-500">
                      <span class="font-semibold">Prozentualer Anteil:</span> {reduction.percent}%
                    </p>
